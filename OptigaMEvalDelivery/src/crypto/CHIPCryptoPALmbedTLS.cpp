...
CHIP_ERROR DRBG_get_bytes(uint8_t * out_buffer, const size_t out_length)
{
    ...
    result = mbedtls_hardware_poll( NULL, out_buffer, out_length, &hw_out_length);
    ...
    IFX_DBG("\n");
		...
		VerifyOrExit(hw_out_length == out_length, error = CHIP_ERROR_INTERNAL);
    VerifyOrExit(result == 0, error = CHIP_ERROR_INTERNAL);
...
CHIP_ERROR P256Keypair::ECDSA_sign_msg(const uint8_t * msg, const size_t msg_length, P256ECDSASignature & out_signature)
{
    ...
    mbedtls_ecp_keypair_init(&keypair);
	  ...
	  IFX_DBG("IFX_>ECDSA_sign_msg() entry...\n");
    ...
    //OptigaM version of mbedtls_ecdsa_sign() uses private key within OptigaM. 
    //d parameter contains OID within OptigaM rather than explicit value
    //For testing, the keypair is generated on Optiga in the test harness
  	IFX_DBG("IFX_>mbedtls_ecdsa_sign() call\n");
    result = mbedtls_ecdsa_sign( &keypair.grp, &r, &s, &keypair.d, hash, sizeof(hash),  CryptoRNG, NULL);
    IFX_DBG("IFX_>mbedtls_ecdsa_sign() returned=>%0x ...\n", result);
    VerifyOrExit(result == 0, error = CHIP_ERROR_INTERNAL);
		

    // Put the values into the DER encoded signature by hand - mbedTLS routines are painful to use. 
		// Format: 0x30|b1|0x02|b2|r|0x02|b3|s, where b1 = Length of remaining data, b2 = Length of r, b3 = Length of s 
    // TBD: Check no leading zero problems.
                                                       ...
                                                       const P256ECDSASignature & signature) const
{
    ...
    mbedtls_mpi_init( &s );
		...
		// Pull the values out of the DER encoded signature by hand - mbedTLS routines are painful to use
		// Format: 0x30|b1|0x02|b2|r|0x02|b3|s, where b1 = Length of remaining data, b2 = Length of r, b3 = Length of s 
    // TBD: Check no leading zero problems. 
    r_len = signature[3];
    s_len = signature [4 + r_len + 1];  
    VerifyOrExit((0x30 == signature[0]), error = CHIP_ERROR_INVALID_SIGNATURE);
    VerifyOrExit((signature.Length()-2 == signature[1]), error = CHIP_ERROR_INVALID_SIGNATURE);
    VerifyOrExit((0x02 == signature[2]), error = CHIP_ERROR_INVALID_SIGNATURE);
    result = 	mbedtls_mpi_read_binary(&r,  &signature[4], r_len);
    VerifyOrExit(result == 0, error = CHIP_ERROR_INVALID_SIGNATURE);
    VerifyOrExit((signature[r_len+4] == 0x02), CHIP_ERROR_INVALID_SIGNATURE);
    result = 	mbedtls_mpi_read_binary(&s,  &signature[4+r_len+2], s_len);
    VerifyOrExit(result == 0, error = CHIP_ERROR_INVALID_SIGNATURE);
    VerifyOrExit((signature.Length() == (size_t) (6 + r_len + s_len)), error = CHIP_ERROR_INVALID_SIGNATURE);
 
    IFX_DBG("IFX_>mbedtls_ecdsa_verify() call...\n");
    result = mbedtls_ecdsa_verify( &keypair.grp, hash, sizeof(hash), &keypair.Q, &r, &s);
    IFX_DBG("IFX_>mbedtls_ecdsa_verify() returned=>%0x ...\n", result);
...
// Note that IFX Optiga version uses OID address in private key data
